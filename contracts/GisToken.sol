...
contract GisToken is AccessControl {
    ...
    modifier onlyBurner {
        require(hasRole(BURNER_ROLE, msg.sender), "don't have burner role");
        _;
    }
    ...
    function mint(address account, uint256 amount) onlyMinter processable(account, amount) public virtual {
        _balances[account] += amount;
        totalSupply += amount;
        emit Transfer(address(0), account, amount);
    }
    
    function burn(address account, uint256 amount) onlyBurner processable(account, amount) public virtual {
